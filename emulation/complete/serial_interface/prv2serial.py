...
class prv2serial:
   def __init__(self, axi_handler: Callable[[axi_request, int], Awaitable[axi_request]]):

      # axi interface     
      self.axi_send_and_recieve: Callable[[axi_request, int], Awaitable[axi_request]] = axi_handler

      # serial interface
      self.serial_io: int = 0
      self.en_i: int = 0
      self.en_o: int = 0

      # serializer
      self.serializer: serializer = serializer(70, 32)
   ...
   def cycle_clock(self, mem_valid: bool, mem_instr: bool, mem_addr: int, mem_wdata: int, mem_wstrb: int, serial_i: bool, en_i: bool):
      ...
      self.serializer.cycle_clock(en_i, serial_i, mem_valid, True, data, msg_length)
   ...
   async def axi_handler_serailizer(self, axi_request_in: axi_request) -> Tuple[axi_request, int]:

      # axi stuff
      self.mem_valid = axi_request_in.mem_valid
      self.mem_instr = axi_request_in.mem_instr
      self.mem_addr = axi_request_in.mem_addr
      self.mem_wdata = axi_request_in.mem_wdata
      self.mem_wstrb = axi_request_in.mem_wstrb

      self.serial_i = axi_request_in,
      self.en_i = axi_request_in
